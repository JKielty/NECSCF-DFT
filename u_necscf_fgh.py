...
from NECSCF.DFT import integral, mathtool, transgrad, ucpks, ucphf, ucphf_field
import numpy
from numpy import sqrt, trace
from scipy.linalg.blas import zgemm, zgemv
from NECSCF.Cphf import ucphf, ucphf_field
...
# Vector potential for nuclear equation
def vec_A(S, C, U1, S1, mo_occ):
...
def make_vec_pon(S, Sn, C, U1, mo_occ, mass):
    vec_pon = 1./mass * vec_A(S, C, U1, Sn, mo_occ)
    return vec_pon
...
def make_FKSeff(ks, mol, aCL, aCR, aU1, aU1_trans, amo_occ, bU1, bU1_trans, bCL, bCR, bmo_occ, grad_nuc, DisVector, mass):
    ...
    bnecoup = make_necoup(mol, S, Sn, bCL, bCR, bU1_trans, bmo_occ, grad_nuc, mass)
    avec_pon = make_vec_pon(S, Sn, aCR, aU1_trans, amo_occ, mass)
    bvec_pon = make_vec_pon(S, Sn, bCR, bU1_trans, bmo_occ, mass)
    vec_pon = avec_pon + bvec_pon
    ...
    return (aFKS - anecoup), (bFKS - bnecoup), anecoup, bnecoup, vec_pon, evec_pon
...
def SCF_iter(mol, aCL, bCL, aCR, bCR, amo_occ, bmo_occ, aU1, bU1, grad_nuc, DisVector, mass):
    ...
    aFKSeff, bFKSeff, anecoup, bnecoup, vec_pon, evec_pon = make_FKSeff(ks, mol, aCL, aCR, aU1, aU1_trans, amo_occ, bU1, bU1_trans, bCL, bCR, bmo_occ, grad_nuc, DisVector, mass)
    ...
    return aCL, bCL, aCR, bCR, amo_energy, bmo_energy, amo_occ, bmo_occ, e_tot, e1, e2, e_coup, e_nuc, vec_pon, MICRO_cycle
...
def SCF_microiter(mol, aCL, bCL, aCR, bCR, amo_occ, bmo_occ, aU1, bU1, grad_nuc, DisVector, mass, micro_tol=1.0e-6, micro_cycle=20, cptype = 'HF'):
    ...
    while not MICRO_conv and MICRO_cycle <= max(1, micro_cycle):
        ...
        MO_ENERGY = (amo_energy, bmo_energy)
        MO_COEFF = (aCR, bCR)
        amo_occ, bmo_occ = dft.uks.UKS(mol).get_occ(MO_ENERGY, MO_COEFF)
        adm = mathtool.odm(aCL, aCR, amo_occ)
        bdm = mathtool.odm(bCL, bCR, bmo_occ)
        e_tot, e1, e2, e_coup, e_nuc = make_energy(ks, mol, h1e, anecoup, bnecoup, adm, bdm)
        e_tot = e_tot + evec_pon
