...
X_k = np.array((np.array(range(N)) - int(NQ - 1))*step/1.88972612) + 1.45
Geom_k = geom.geom_gen(Q, DisVector, N, NQ, step/1.88972612)

print('Molecular Geometries:\n', np.array(Geom_k).real)

aCL, bCL, aCR, bCR, amo_energy, bmo_energy, amo_occ, bmo_occ, en_energy, nuc_wfn, NUC_GRAD = u_scfcycle_fgh.necscf_run(mol, Q, DisVector, N, NQ, step, v, multi = 25, micro=True, egy_tol=1.0e-4, rms_tol=1.0e-3, maxcycle=20, micro_tol=1.0e-6, micro_cycle=20, cptype = 'HF')

plt.plot(X_k, en_energy, 'ko', X_k, en_energy, 'k--', color = 'red', label='NECSCF-DFT')
plt.xlabel('Displacement / a.u.')
plt.ylabel('Energy / Hartree')
plt.legend()
plt.savefig(f'PESs_v{v}Micro280Def2.png')
plt.close()
plt.plot(X_k, en_energy, 'ko', X_k, en_energy, 'k--', color = 'red')
plt.xlabel('Displacement / a.u.')
plt.ylabel('Energy / Hartree')
plt.savefig(f"Final280MicroPES_v{v}Def2.png")
...
plt.legend()
plt.xlabel('Displacement / a.u.')
plt.savefig(f'Final_NUCMicrov{v}Def2.png')
plt.close()