...
def make_f1(mol, amocc, bmocc, amocct, bmocct, amvir, bmvir, amvirt, bmvirt, atm_no, dofdes):
    ...
    h1c = -numpy.array([mol.intor('cint1e_irp_sph',comp=9)[6], \
                       mol.intor('cint1e_irp_sph',comp=9)[7], \
                       mol.intor('cint1e_irp_sph',comp=9)[8]])
    h1c = integral.int1e3_dof(mol, h1c, atm_no, dofdes)
    h1a = h1a - 1*mol.atom_charge(atm_no) * h1b -h1c                                                                                     # Combine hcore(0) with hcore(1)
...
def make_B0(mol, amocc, bmocc, amocct, bmocct, amvir, bmvir, amvirt, bmvirt, amo_energy, bmo_energy, amo_occ, bmo_occ, atm_no, dofdes):
    ...
    # Fixing numerical issues from near degenerate orbitals:
...
def make_U(mol, aCL, bCL, aCR, bCR, amo_energy, bmo_energy, amo_occ, bmo_occ, rms_tol=1.0e-15, maxcycle=1000):
    lindep=1.0e-15
    natm = mol.natm
